...
| `scripts/download-models.sh` | — | Sketchfab 3D aircraft model downloader (`design/aircraft-reference/models/`, dev-box-only). Run it **directly** with no args for an interactive "enter a number" picker. `./gradlew downloadModels --model=<number\|short-name\|full-name>` / `--all` also work but **can't prompt** under the normal daemon-backed invocation (no TTY) → lists models when no `--model` given. |
| `scripts/download-models-interactive.sh` | `npm run models:pick` | Runs `downloadModels` via `./gradlew --no-daemon` so the picker prompt works through the Gradle task surface (slower startup than running `download-models.sh` directly — no daemon reuse — but discoverable via `./gradlew tasks --group game`). |
