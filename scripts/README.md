...
| `scripts/download-models.sh` | — | Sketchfab 3D aircraft model downloader (`design/aircraft-reference/models/`, dev-box-only). Run it **directly** with no args for an interactive "enter a number" picker. `./gradlew downloadModels --model=<number\|short-name\|full-name>` / `--all` also work but **can't prompt** (no TTY) → lists models when no `--model` given. |
