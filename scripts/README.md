...
| `scripts/download-models.sh` | — | Sketchfab 3D aircraft model downloader (`design/aircraft-reference/models/`, dev-box-only). Automated once a Sketchfab OAuth app is set up (one-time — see `models/README.md`); falls back to manual-download instructions otherwise. Run it **directly** with no args for an interactive "enter a number" picker. Plain `./gradlew downloadModels --model=<number\|short-name\|full-name>` / `--all` also work but **can't prompt** on their own (the Gradle build daemon has no controlling terminal) → lists models when no `--model` given. |
