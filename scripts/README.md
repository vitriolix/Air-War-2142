...
| `scripts/blender.sh` | `npm run blender:open` | Launch Blender without needing its install path. `--headless` for no GUI. Pass a model name (fuzzy-matched against downloaded folders in `design/aircraft-reference/models/`) to load it — auto-imports `scene.gltf` and switches every viewport to Material Preview shading if no `.blend` exists yet for that model (Blender's default Solid shading never shows textures). `./gradlew launchBlender` also works for the no-args case. |
